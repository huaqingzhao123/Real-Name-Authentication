...
/// </summary>
public class IllegalWordDetection : Singleton<IllegalWordDetection>
{
    ...
    IllegalWordDetection() { }
    public void ReadText(TextAsset text)
    {
        var badWordsText = text.text;
        badWords = badWordsText.Split(new char[] { ',', '，', '、', '、' });
        Init(badWords);
        Debug.Log("读取成功");
    }

    //public string Checked(string content)
    //{
    //    //StringSearch iwords = new StringSearch();
    //    //iwords.SetKeywords(badWords);
    //    IllegalWordsSearch illegal = new IllegalWordsSearch();
    //    illegal.SetKeywords(badWords);
    //    return illegal.Replace(content);
    //}
